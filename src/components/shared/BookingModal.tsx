    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      ...
      >
        ...
        <div className="overflow-y-auto max-h-[94vh] sm:max-h-[92vh] mt-2 sm:mt-0 pb-4 sm:pb-0">
          ...
          {step === 1 && (
            <>
              <div className="px-4 sm:px-4 pt-5 sm:pt-6 pb-5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <DialogTitle className="font-heading font-bold leading-tight" style={{ fontSize: "clamp(1.1rem,3vw,1.4rem)", letterSpacing: "-0.025em", color: C.text }}>
                      Book a Service
                    </DialogTitle>
                    <p className="mt-1 font-body text-[11px]" style={{ color: C.textMuted }}>Choose how you&apos;d like to get started</p>
                  </div>
                  <CloseBtn onClose={handleClose} />
                </div>
              </div>
              <div className="px-4 pb-4 flex flex-col gap-2">
                <OptionCard onClick={chatDirectly} iconBg="linear-gradient(135deg,#dcfce7,#bbf7d0)" iconColor="#15803d" icon={<MessageCircle className="h-3 w-3" />} title="Chat Directly" subtitle="Open WhatsApp and talk to our team right away" accentHoverBorder="rgba(21,128,61,0.3)" accentHoverBg="rgba(21,128,61,0.04)" />
                <OptionCard onClick={() => setStep(2)} iconBg="linear-gradient(135deg,#fef3c7,#fde68a)" iconColor="#b45309" icon={<Wrench className="h-3 w-3" />} title="Define a Problem" subtitle="Select your appliance, describe the issue & book instantly" accentHoverBorder={C.borderGold} accentHoverBg={C.goldLight} />
              </div>
          ...
          {step === 2 && (
            <div className="px-4 pb-4 pt-3 flex flex-col gap-2">
              <p className="font-heading font-bold" style={{ fontSize: "clamp(0.95rem,2.5vw,1.2rem)", letterSpacing: "-0.02em", color: C.text }}>What needs to be repaired?</p>
              ...
              <button
                ...
                className="w-full flex items-center gap-1.5 rounded-xl px-4 py-4 transition-all duration-300"
                style={{
              ...
              >
                <div className="h-8 w-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: OTHER_APPLIANCE.iconBg, color: OTHER_APPLIANCE.iconColor }}>
                  <span className="h-3 w-3 flex items-center justify-center [&>svg]:h-full [&>svg]:w-full">{OTHER_APPLIANCE.icon}</span>
                </div>
                <div className="flex-1 text-left">
                  <p className="font-body font-bold text-[11px]" style={{ color: appliance?.id === "other" ? C.navy : C.text }}>Other Appliance / Issue</p>
                  <p className="font-body text-[10px] mt-0.5" style={{ color: C.textMuted }}>Tell us what you need — we repair most home appliances</p>
                </div>
                {appliance?.id === "other" && <CheckCircle2 className="h-3 w-3 flex-shrink-0" style={{ color: C.gold }} />}
              </button>
          ...
          {step === 3 && appliance && !appliance.isOther && (
            <div className="px-4 pb-4 pt-3 flex flex-col gap-2">
              <p className="font-heading font-bold" style={{ fontSize: "clamp(0.95rem,2.5vw,1.2rem)", letterSpacing: "-0.02em", color: C.text }}>About your {appliance.label}</p>
              <div className="flex flex-col gap-1.5">
                <label className="font-body text-[11px] font-semibold" style={{ color: C.text }}>Model name <span className="font-normal" style={{ color: C.textMuted }}>(optional)</span></label>
                <LightInput placeholder="e.g. Voltas 1.5 Ton Inverter" value={model} onChange={setModel} />
              </div>
              <div className="flex flex-col gap-2">
                <label className="font-body text-[11px] font-semibold" style={{ color: C.text }}>What&apos;s the problem? <span className="font-normal" style={{ color: C.textMuted }}>(select all that apply)</span></label>
                <div className="flex flex-wrap gap-2">
                  {appliance.problems.map((p) => {
                    const on = selectedProblems.includes(p);
                    return (
                      <button key={p} onClick={() => toggleProblem(p)} className="rounded-full px-3.5 py-2 font-body text-[11px] font-semibold transition-all duration-200"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="font-body text-[11px] font-semibold" style={{ color: C.text }}>Anything else? <span className="font-normal" style={{ color: C.textMuted }}>(optional)</span></label>
          ...
          {step === 4 && appliance?.isOther && (
            <div className="px-4 pb-4 pt-3 flex flex-col gap-2">
              <p className="font-heading font-bold" style={{ fontSize: "clamp(0.95rem,2.5vw,1.2rem)", letterSpacing: "-0.02em", color: C.text }}>Tell us what you need</p>
              <div className="flex flex-col gap-1.5">
                <label className="font-body text-[11px] font-semibold" style={{ color: C.text }}>What appliance needs repair?</label>
                <LightInput placeholder="e.g. Water heater, Geyser, Microwave…" value={model} onChange={setModel} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="font-body text-[11px] font-semibold" style={{ color: C.text }}>Describe the problem</label>
          ...
          {((step === 4 && !appliance?.isOther) || step === 5) && (
            <div className="px-4 pb-5 pt-3 flex flex-col gap-2">
              <p className="font-heading font-bold" style={{ fontSize: "clamp(0.95rem,2.5vw,1.2rem)", letterSpacing: "-0.02em", color: C.text }}>Your details</p>
              {appliance && (
                <div className="rounded-xl p-4 flex items-center gap-2" style={{ background: C.goldLight, border: `1px solid ${C.goldBorder}` }}>
                  <div className="flex-shrink-0 h-8 w-8 rounded-xl flex items-center justify-center" style={{ background: appliance.iconBg, color: appliance.iconColor }}>
                    <span className="h-3 w-3 flex items-center justify-center [&>svg]:h-full [&>svg]:w-full">{appliance.icon}</span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-body font-bold text-[11px]" style={{ color: C.text }}>
                    ...
                    <p className="font-body text-[10px] mt-0.5 truncate" style={{ color: C.textSub }}>
                      {[...selectedProblems, extraNote].filter(Boolean).join(" · ") || "Issue described above"}
                    </p>
                  </div>
                  <Zap className="h-3 w-3 flex-shrink-0" style={{ color: C.gold }} />
                </div>
              )}
              <div className="flex flex-col gap-1.5">
                <label className="font-body text-[11px] font-semibold flex items-center gap-2" style={{ color: C.text }}><User className="h-3 w-3" style={{ color: C.gold }} /> Your full name</label>
                <LightInput placeholder="e.g. Raj Patel" value={name} onChange={setName} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="font-body text-[11px] font-semibold flex items-center gap-2" style={{ color: C.text }}><Phone className="h-3 w-3" style={{ color: C.gold }} /> Phone number</label>
                <LightInput placeholder="e.g. 98765 43210" value={phone} onChange={setPhone} type="tel" />
              </div>
              <div className="flex gap-2 pt-1">
                <GhostBtn onClick={() => appliance?.isOther ? setStep(4) : setStep(3)}><ArrowLeft className="h-3 w-3" /> Back</GhostBtn>
                <button onClick={bookService} disabled={!name.trim() || !phone.trim()} className="flex-1 flex items-center justify-center gap-1.5 rounded-xl h-9 font-body font-bold text-[11px] transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed"
                  ...
                  <MessageCircle className="h-3 w-3" />Book My Service
                </button>
              </div>
              <p className="text-center font-body text-[10px]" style={{ color: C.textMuted }}>Your details are pre-filled in WhatsApp — just hit send 🚀</p>
            </div>
          )}
...
function ApplianceCard({ a, selected, onClick }: { a: Appliance; selected: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1.5 rounded-xl p-2.5 transition-all duration-300"
      style={{
      ...
      <div className="h-9 w-9 rounded-xl flex items-center justify-center" style={{ background: a.iconBg, color: a.iconColor }}>
        <span className="h-3 w-3 flex items-center justify-center [&>svg]:h-full [&>svg]:w-full">{a.icon}</span>
      </div>
      <p className="font-body font-bold text-center leading-tight text-[11px]" style={{ color: selected ? C.navy : C.text }}>{a.label}</p>
      <p className="font-body text-center leading-tight text-[9px]" style={{ color: C.textMuted }}>{a.sublabel}</p>
      {selected && <CheckCircle2 className="h-3 w-3 mt-1" style={{ color: C.gold }} />}
    </button>
  );
}
...
function StepHeader({ step, appliance, onClose }: { step: number; appliance: Appliance | null; onClose: () => void }) {
  ...
  return (
    <div className="sticky top-0 z-20 px-4 pt-4 sm:pt-3 pb-2.5" style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
      <div className="flex items-center justify-between gap-2 mb-2.5">
        <div className="flex items-center gap-2">
          {appliance && step > 2 && (
            <div className="h-3 w-3 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: appliance.iconBg, color: appliance.iconColor }}>
              <span className="h-3 w-3 flex items-center justify-center [&>svg]:h-full [&>svg]:w-full">{appliance.icon}</span>
            </div>
          )}
          <div>
            <DialogTitle className="sr-only">Book a Service</DialogTitle>
            <p className="font-body font-bold text-[10px]" style={{ color: C.text }}>
            ...
            <p className="font-body text-[10px]" style={{ color: C.textMuted }}>Step {stepIndex} of {totalSteps}</p>
          </div>
        </div>
          ...
          return (
            <div key={i} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-1 flex-shrink-0">
                <div className="rounded-full flex items-center justify-center transition-all duration-500 font-body font-bold" style={{ width: active ? "20px" : "16px", height: active ? "20px" : "16px", background: done || active ? "linear-gradient(135deg,hsl(37,90%,50%),hsl(30,95%,44%))" : C.border, color: done || active ? "hsl(216,50%,10%)" : C.textMuted, fontSize: "0.5rem" }}>
                  {done ? "✓" : i + 1}
                </div>
                <span className="font-body font-semibold" style={{ fontSize: "0.45rem", color: active ? C.gold : done ? C.textSub : C.textMuted }}>{labels[i]}</span>
              </div>
              {i < totalSteps - 1 && <div className="flex-1 h-px mx-1.5 mb-3 transition-all duration-700" style={{ background: done ? "linear-gradient(90deg,hsl(37,90%,50%),hsl(30,95%,44%))" : C.border }} />}
            </div>
          );
        })}
      </div>
...
function CloseBtn({ onClose, light = false }: { onClose: () => void; light?: boolean }) {
  return (
    <button onClick={onClose} aria-label="Close dialog" className="flex-shrink-0 flex h-3 w-3 items-center justify-center rounded-xl transition-all duration-200" style={{ background: light ? C.border : "#edf0f7", color: C.textSub }}>
      <X className="h-3 w-3" />
    </button>
  );
}
...
function OptionCard({ onClick, iconBg, iconColor, icon, title, subtitle, accentHoverBorder, accentHoverBg }: { onClick: () => void; iconBg: string; iconColor: string; icon: React.ReactNode; title: string; subtitle: string; accentHoverBorder: string; accentHoverBg: string }) {
  return (
    <button onClick={onClick} className="group w-full text-left rounded-xl p-3 flex items-center gap-1.5 transition-all duration-300 hover:-translate-y-0.5" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
      <div className="flex-shrink-0 h-10 w-10 rounded-xl flex items-center justify-center" style={{ background: iconBg, color: iconColor }}>
        <span className="h-3 w-3 flex items-center justify-center [&>svg]:h-full [&>svg]:w-full">{icon}</span>
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-heading font-bold text-[13px]" style={{ color: C.text }}>{title}</p>
        <p className="font-body text-[11px] mt-0.5" style={{ color: C.textSub }}>{subtitle}</p>
      </div>
      <ArrowRight className="h-3 w-3 flex-shrink-0 transition-transform duration-300 group-hover:translate-x-1" style={{ color: C.textMuted }} />
    </button>
  );
}
...
function LightInput({ placeholder, value, onChange, type = "text" }: { placeholder: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <input type={type} placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} className="w-full rounded-xl px-3 h-9 font-body text-[10px] outline-none transition-all duration-300 focus:ring-1 focus:ring-yellow-400/50" style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.text }} />
  );
}
...
function LightTextarea({ placeholder, value, onChange, rows }: { placeholder: string; value: string; onChange: (v: string) => void; rows: number }) {
  return (
    <textarea rows={rows} placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} className="w-full rounded-xl px-3 py-2 font-body text-[10px] outline-none transition-all duration-300 resize-none focus:ring-1 focus:ring-yellow-400/50" style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.text }} />
  );
}
...
function NavButtons({ onBack, onNext, nextDisabled, nextLabel }: { onBack: () => void; onNext: () => void; nextDisabled: boolean; nextLabel: string }) {
  return (
    <div className="flex gap-2 pt-0.5">
      <GhostBtn onClick={onBack}><ArrowLeft className="h-3 w-3" /> Back</GhostBtn>
      <button onClick={onNext} disabled={nextDisabled} className="flex-1 flex items-center justify-center gap-1.5 rounded-xl h-9 font-body font-bold text-[10px] transition-all duration-300 disabled:opacity-35 disabled:cursor-not-allowed hover:-translate-y-0.5"
...
function GhostBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className="flex items-center gap-1.5 rounded-xl px-3 h-9 font-body font-semibold text-[10px] transition-all duration-300 hover:brightness-95" style={{ background: "#edf0f7", border: `1px solid ${C.border}`, color: C.textSub }}>
      {children}
    </button>
  );
}
